...
scopeName "TB_fnc_loadInventory_whiteList";
...
if (_nameID >= 0) then {
	...
	["Inventory '%1' not found",_name] call BIS_fnc_error; breakOut "TB_fnc_loadInventory_whiteList";
};
...
if !(local _object) exitWith {[[_object,_cfg,_whiteList],"TB_fnc_loadInventory_whiteList",_object] call BIS_fnc_MP; false};
...
if ( _uniform != "" ) then {
	if ( ( toLower _uniform ) in _whiteList ) then {
...
//VEST
if ( _vest != "" ) then {
	if ( ( toLower _vest ) in _whiteList ) then {
...
//HEADGEAR
if ( _headgear != "" ) then {
	if ( ( toLower _headgear ) in _whiteList ) then {
...
//GOGGLES
if ( _goggles != "" ) then {
	if ( ( toLower _goggles ) in _whiteList ) then {
...
if ( _backpack != "" ) then {
	if ( ( toLower _backpack ) in _whiteList ) then {
...
{
	if (_x != "" ) then {
		if ( ( toLower _x ) in _whiteList ) then {
