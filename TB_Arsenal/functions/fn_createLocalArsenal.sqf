...
_obj removeAction (_obj getVariable ["TB_actionID", -1]);
terminate (_obj getVariable ["TB_restrictionHandle", scriptNull]);
...
for "_i" from 0 to (count _specTypes - 1) do {
	...
	if ((toLower str _typeArray) find (toLower typeOf player)> -1 or (toLower str _typeArray) find (toLower str player)> -1) exitWith {
...
_obj setVariable ["TB_specNames", _specNames];
_obj setVariable ["TB_specItems", _specItems];
...
_handle = [_obj, _restrictionDistance] spawn TB_fnc_invRestriction;
_obj setVariable ["TB_restrictionHandle", _handle];
